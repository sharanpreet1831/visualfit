...
class SummaryViewController: UIViewController {
    ...
    func updateUI() {
        ...
        if let image = userInstance.user.todayImage{
            beforeImage.image = image
        }
        else{
            print("here")
            beforeImage.image = UIImage(named: "josh-before")
        }
        
        if let image = userInstance.user.todayTransformedImage{
            afterImage.image = image
        }
        else{
            print("isme bhi here")
            afterImage.image = UIImage(named:"josh-after")
            afterImage.tintColor = .primary
        }
